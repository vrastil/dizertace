...
%DIF DEL thesis-old-flat.tex   Mon Sep 14 14:01:03 2020
%DIF ADD thesis-new-flat.tex   Mon Sep 14 16:31:35 2020
...
%DIF 71a71
\usepackage[stable]{footmisc} %DIF > 
%DIF -------
...
%DIF 124a125-128
\makeatletter %DIF > 
\newcommand\footnoteref[1]{\protected@xdef\@thefnmark{\ref{#1}}\@footnotemark} %DIF > 
\makeatother %DIF > 
 %DIF > 
%DIF -------
\newcommand{\T}[1]{#1^\top}
...
\crefformat{footnote}{\footnotemark[#1]} %DIF > 
%DIF -------
...
In~\DIFdelbegin %DIFDELCMD < \autoref{chpt:cosmo_surveys} %%%
\DIFdel{we review present-day and~future experiments designed to~study our Universe. In~}\DIFdelend the~end, in~\autoref{chpt:outlook} we discuss possible future applications of~implemented techniques and~ways to~further improve them.
...
The~standard cosmological model is based on~the~\textit{cosmological principle}, first clearly formulated in~\textcite{1687pnpm.book.....N}. The~cosmological principle states that the~Universe is homogeneous and~isotropic on~sufficiently large scales\DIFdelbegin \DIFdel{(beyond those traced by the~large-scale structure of~the~distribution of~galaxies)}\DIFdelend . The~cosmic microwave background (CMB) observations strongly support this statement as photons from different parts of~the~Universe are coming with~almost identical temperatures. \DIFdelbegin \DIFdel{This isotropy, however, }\DIFdelend \DIFaddbegin \DIFadd{However, the isotropy }\DIFaddend does not imply homogeneity. The~\textit{Copernican Principle} states that the~observer is not in~a~special place or time. With~the~observed isotropy, the~Copernican Principle implies the~Cosmological Principle. The~observed inhomogeneities and~irregularities on~local scales come from gravitational instability and~do not violate these principles.
...
In~\DIFdelbegin \DIFdel{addition to~the ~cosmic time $t$, we also introduce }\DIFdelend \DIFaddbegin \DIFadd{the work, we use also }\DIFaddend the~conformal time $\eta$ defined \DIFdelbegin \DIFdel{by
}\DIFdelend \DIFaddbegin \DIFadd{as
}\DIFaddend \eq{
    \eta\equiv\int{a^{-1}\dd t}\,.
}
\DIFdelbegin \DIFdel{The~dynamical }\DIFdelend \DIFaddbegin \DIFadd{From the~Einstein }\DIFaddend equations of~motion in~the~expanding Universe \DIFdelbegin \DIFdel{can be derived from the~Einstein equations resulting in~}\DIFdelend \DIFaddbegin \DIFadd{one can derive }\DIFaddend the~Friedmann equations
\eq{
    \label{eq:Friedmann}
...
\DIFdelbegin \DIFdel{The~pressure is related to~a~force associated with~a~local potential that depends on~the~position in~space but in~a~homogeneous and~isotropic universe there is no such potential.
}\DIFdelend 
...
\DIFdelbegin \DIFdel{In~an~expanding Universe, a~physical }\DIFdelend \DIFaddbegin \DIFadd{Physical }\DIFaddend distance $r$ from an~observer \DIFdelbegin \DIFdel{(at~the~origin) }\DIFdelend to~an~object \DIFaddbegin \DIFadd{in~an~expanding Universe }\DIFaddend is given by $r=a(t)x$, where $x$ denotes the~comoving distance. \DIFdelbegin \DIFdel{For~objects which are motionless }\DIFdelend \DIFaddbegin \DIFadd{Objects, which are at~rest }\DIFaddend with~respect to~the~Hubble flow, \DIFdelbegin \DIFdel{the~comoving distanceremains constant}\DIFdelend \DIFaddbegin \DIFadd{have constant comoving distance}\DIFaddend . Taking the~time derivative of~$r$ we obtain
\eq{
...
where $\Psi(t,x)$ and~$\Phi(t,x)$ are spatial scalars, $w_i(t,x)$ is 3-vector, and~$h_{ij}(t,x)$ is a~traceless 3-tensor. We have the~freedom to~choose coordinates in~which we will describe the~equations of~motion -- we can choose our \textit{gauge}. We wish to~keep our background metric $g_{\mu\nu}^{(0)}$ the~same (i.e. FLRW) and~only change $\delta g_{\mu\nu}$. Note that unlike ordinary coordinate transformations\DIFdelbegin \DIFdel{a~gauge transformation (although expressed as a~change of~coordinates) does not }\DIFdelend \DIFaddbegin \DIFadd{, which }\DIFaddend link different observers in~the~same spacetime\DIFdelbegin \DIFdel{but it }\DIFdelend \DIFaddbegin \DIFadd{, a~gauge transformation }\DIFaddend links two different spacetimes seen by the~same observer. For~a~detailed discussion of~gauge choices, see e.g. \textcite{PhysRevD.40.1804,10.1143/PTPS.78.1,PhysRevD.22.1882}.
...
where vertical bars represent the~change of~quantities between $e$ and~$o$. The~first \DIFdelbegin \DIFdel{effect is referred to~as }\DIFdelend \DIFaddbegin \DIFadd{term is called }\DIFaddend the~\textit{Sachs--Wolfe effect} and~depends on~the~difference between the~potential at~the~time of~emission and~observation. The~second \DIFdelbegin \DIFdel{effect }\DIFdelend \DIFaddbegin \DIFadd{term }\DIFaddend is called the~\textit{integrated Sachs--Wolfe effect} and~depends on~the~line-of-sight integral of~$\Phi-\Psi$.
...
In~the~Einstein--de Sitter Universe, we can compute the~critical \DIFdelbegin \DIFdel{or collapse }\DIFdelend value $\delta\coll$ of~the~linear fluctuation that is reached at~the~time of~the~\DIFaddbegin \DIFadd{non-linear spherical }\DIFaddend collapse. This quantity \DIFdelbegin \DIFdel{is an~important result of~this model and~it has cosmological relevance as }\DIFdelend \DIFaddbegin \DIFadd{represents }\DIFaddend a~first approximation to~the~epoch of~galaxy formation and~\DIFaddbegin \DIFadd{can be~used }\DIFaddend to~calculate the~abundance of~collapsed objects. A~spherical perturbation \DIFdelbegin \DIFdel{in~the~Einstein-–de Sitter Universe collapses }\DIFdelend \DIFaddbegin \DIFadd{will collapse }\DIFaddend to~a~singularity whenever the~linear density contrast $\delta_L=\delta\coll\approx1.686$.
...
Here we present a~brief description of~cosmological probes that can be used for~constraining dark energy. For~more details see e.g. \textcite{weinberg_observational_2013}\DIFaddbegin \DIFadd{, \textcite{2010deto.book.....A}, }\DIFaddend or \textcite{DE_probes2}.

\DIFaddbegin \DIFadd{We have already studied this topic and~some of~the~presented probes in~the~author`s work \textcite{mastersthesis_vrastil}. Although in~the~main part of the work we deal mainly with large-scale structures and baryonic acoustic oscillations, general overview of~other~probes might interests some readers. Therefore, we reuse some parts of the~former text here (written in italics). Explicitly, this concerns sub-sections }\hyperref[ssec:supernovae]{Supernovae}\DIFadd{, }\hyperref[sec:bao]{Baryonic acoustic oscillations}\DIFadd{, }\hyperref[ssec:wl]{Weak lensing}\DIFadd{, }\hyperref[ssec:lss]{Large-scale structure}\DIFadd{, }\hyperref[ssec:gc]{Galaxy clusters}\DIFadd{, }\hyperref[ssec:SL]{Strong lensing}\DIFadd{, and }\hyperref[sec:rsd]{Redshift-space distortions}\DIFadd{.
}

\DIFaddend \subsection{Cosmic distances}
...
The~luminosity distance $d_L$ is used in~supernovae observations to~link the~supernova luminosity with~\DIFdelbegin \DIFdel{the~expansion rate of~the~Universe}\DIFdelend \DIFaddbegin \DIFadd{its~distance}\DIFaddend . It is defined \DIFdelbegin \DIFdel{by
}\DIFdelend \DIFaddbegin \DIFadd{as }\parencite{weinberg_observational_2013}
\DIFaddend \eq{
...
The~cosmic microwave background (CMB) is the~oldest $(z\sim1090)$ electromagnetic radiation we can see and~its surface forms the~so-called last scattering surface. The~photons were tightly coupled to~baryons and~electrons before this decoupling epoch in~a~hot plasma and~they could not propagate freely. \DIFdelbegin \DIFdel{\textcite{1965ApJ...142..419P} first detected }\DIFdelend \DIFaddbegin \DIFadd{The first detection of }\DIFaddend the~CMB photons \DIFaddbegin \DIFadd{was performed by \textcite{1965ApJ...142..419P}. They observe the radiation }\DIFaddend thermalized to~an~almost uniform temperature across the~\DIFaddbegin \DIFadd{whole }\DIFaddend sky. The \DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{predicted }\DIFaddend temperature anisotropies of~the~CMB were first measured at~large angular separations by the~COBE satellite in~\textcite{1992ApJ...396L...1S}. The~precise measurement of~temperature anisotropies by high-precision experiments \parencite[e.g.][]{2003ApJS..148..175S} \DIFdelbegin \DIFdel{opened up }\DIFdelend \DIFaddbegin \DIFadd{provided }\DIFaddend a~new \DIFdelbegin \DIFdel{opportunity }\DIFdelend \DIFaddbegin \DIFadd{direction how }\DIFaddend to~determine cosmological parameters\DIFdelbegin \DIFdel{to~high precision}\DIFdelend .

The~CMB provides critical information regarding our Universe. It can constrain parameters such as $\Omega_m,\ \Omega_k$, the~high-redshift normalization of~matter fluctuations, the~spectral index $n_s$ and~curvature $\dd n_s/\dd \ln k$ of~the~scalar fluctuation spectrum, the~amplitude and~slope of~the~tensor (gravitational wave) fluctuation spectrum, the~post-recombination electron-scattering optical depth $\tau$, and~the~Hubble constant \DIFaddbegin \parencite{weinberg_observational_2013}\DIFaddend .

\DIFdelbegin \DIFdel{In~}\DIFdelend \DIFaddbegin \DIFadd{The origin of CMB is described, in }\DIFaddend most theoretical models, \DIFdelbegin \DIFdel{the~CMB is generated by }\DIFdelend \DIFaddbegin \DIFadd{as }\DIFaddend an~isotropic Gaussian stochastic process\DIFdelbegin \DIFdel{, in~which case all the~available information concerning the ~}\DIFdelend \DIFaddbegin \DIFadd{. For gaussian perturbations, all information about the }\DIFaddend underlying theoretical model \DIFdelbegin \DIFdel{can be extracted by measuring }\DIFdelend \DIFaddbegin \DIFadd{is contained in }\DIFaddend the~angular power spectrum of~the~CMB anisotropies. \DIFdelbegin \DIFdel{Because of~isotropy }\DIFdelend \DIFaddbegin \DIFadd{Due to the isotropy of the process}\DIFaddend , one may expand the~\DIFdelbegin \DIFdel{map }\DIFdelend \DIFaddbegin \DIFadd{CMB }\DIFaddend in~spherical harmonics to~extract its angular power spectrum, defined as \parencite{2015IJMPD..2430004B}
\eq{
...
\begin{figure}[!hbt]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.85\textwidth]{cosmo_evol/CMB_compilation.png}
%DIFDELCMD <     %%%
...
\DIFdelend \DIFaddbegin \subsection[Supernovae]{\DIFaddend Supernovae\DIFaddbegin \footnote{\label{ftn1}\DIFadd{Some parts of this section have already been published in \textcite{mastersthesis_vrastil}. The overtaken text is written in italics.}}\DIFaddend }
\DIFaddbegin \label{ssec:supernovae}
{\itshape
...
\end{figure}
\DIFdelbegin %DIFDELCMD < \subsection{%%%
\DIFdelend \DIFaddbegin \subsection[Baryonic acoustic oscillations]{\DIFaddend Baryonic acoustic oscillations\DIFaddbegin \footnoteref{ftn1}\DIFaddend }
\label{sec:bao}
\DIFaddbegin {\itshape
\DIFaddend Baryonic acoustic oscillations (BAO) provide an~entirely independent way of~measuring cosmic distance. Sound waves propagating before recombination imprint a~characteristic scale on~matter clustering. The~acoustic length scale can be computed as
\eq{
...
\DIFdelbegin %DIFDELCMD < \subsection{%%%
\DIFdelend \DIFaddbegin \subsection[Weak lensing]{\DIFaddend Weak lensing\DIFaddbegin \footnote{\DIFadd{Some parts of this section have already been published in \textcite{mastersthesis_vrastil}. The overtaken text is written in italics.}}\DIFaddend }
...
\subsection[Large-scale structure]{\DIFaddend Large-scale structure\DIFaddbegin \footnote{\DIFadd{Some parts of this section have already been published in \textcite{mastersthesis_vrastil}. The overtaken text is written in italics.}}\DIFaddend }
...
\DIFaddbegin }
\DIFaddend \subsubsection{Matter power spectrum}
...
where $\delta_{\rm D}$ is the~Dirac delta function and~we are averaging over possible realizations. The~power spectrum is \DIFdelbegin \DIFdel{by far the ~most common descriptor of~clustering }\DIFdelend \DIFaddbegin \DIFadd{the most used probe in cosmology }\DIFaddend in~the~linear and~mildly non-linear regime\DIFdelbegin \DIFdel{and~plays a~central role in~cosmology}\DIFdelend . The~power spectrum is the~Fourier transform of~the~correlation function \eqref{eq:corr}
\eq{
    \label{eq:pk_xi}
...
from which follows $P_g(k)=b^2P_m(k)$. \DIFdelbegin \DIFdel{The~idea of~a~simple biasing scheme tries to ~capture physics beyond the~purely linear gravitational treatment, e.g. merging processes }\DIFdelend \DIFaddbegin \DIFadd{This simple assumption tries to incorporate non-linear physical processes, such as merging of galaxies, }\DIFaddend or evolutionary processes \DIFdelbegin \DIFdel{that render }\DIFdelend \DIFaddbegin \DIFadd{of stars that makes }\DIFaddend galaxies brighter or dimmer\DIFdelbegin \DIFdel{and~therefore visible or invisible to~our telescopes}\DIFdelend .
...
\DIFdelbegin %DIFDELCMD < \subsection{%%%
\DIFdelend \DIFaddbegin \subsection[Galaxy clusters]{\DIFaddend Galaxy clusters\DIFaddbegin \footnote{\DIFadd{Some parts of this section have already been published in \textcite{mastersthesis_vrastil}. The overtaken text is written in italics.}}\DIFaddend }
...
\DIFaddbegin }
\DIFaddend \subsubsection{The~halo mass function}
...
where the~parameter \(\delta\coll=1.686\) \DIFaddbegin \DIFadd{was }\DIFaddend introduced in~the~previous section\DIFdelbegin \DIFdel{can be interpreted physically as the~linearly extrapolated overdensity of~a~top-hat spherical density perturbation at~the~moment of~maximum compression for~an~Einstein--de Sitter universe. While as a~first approximation the}\DIFdelend \DIFaddbegin \DIFadd{. The}\DIFaddend ~PS mass function agrees with~simulations at~$z=0$ reasonably well, \DIFaddbegin \DIFadd{but }\DIFaddend it overpredicts the~number of~low-mass halos and~underpredicts the~number of~massive halos at~the~current epoch \DIFaddbegin \parencite{2007ApJ...671.1160L}\DIFaddend . \textcite{2001MNRAS.321..372J} combined high-resolution simulations for~four different CDM cosmologies spanning a~mass range of~over 3 orders of~magnitude $(\sim10^{12}-10^{15}h^{-1}M_\odot)$, and~including several redshifts $z=0-5$. They came up with~the~following fitting function
\eq{
...
\DIFdelbegin %DIFDELCMD < \subsection{%%%
\DIFdelend \DIFaddbegin \subsection[Strong lensing]{\DIFaddend Strong lensing\DIFaddbegin \footnote{\label{ftn2}\DIFadd{Some parts of this section have already been published in \textcite{mastersthesis_vrastil}. The overtaken text is written in italics.}}\DIFaddend }
...
\DIFdelbegin %DIFDELCMD < \subsection{%%%
\DIFdelend \DIFaddbegin }
\subsection[Redshift-space distortions]{\DIFaddend Redshift-space distortions\DIFaddbegin \footnoteref{ftn2}\DIFaddend }
\label{sec:rsd}
\DIFaddbegin {\itshape
\DIFaddend When we observe distant galaxies, two features determine their redshifts -- the~Hubble expansion and~their peculiar velocities. The~peculiar velocities of~galaxies thus cause them to~appear displaced along the~line of~sight in~redshift space. These displacements lead to~redshift distortions in~the~pattern of~clustering of~galaxies in~redshift space and~make large scale galaxy clustering anisotropic. Redshift-space distortions (RSD) have the~tremendous advantage of~bearing information about the~dynamics of~galaxies.\DIFaddbegin }
\DIFaddend 

The~coordinate transformation from real space $(r)$ to~redshift space $(s)$ of~a~source with~a~peculiar velocity $\bm v$ is given by \DIFaddbegin \parencite{2010deto.book.....A}
\DIFaddend \eq{
    \mb s = \mb r\left[1+\frac{u(r)-u(0)}{r}\right]\,,
}
where $u\equiv\mb v\cdot\mb r/r$. \DIFaddbegin {\itshape \DIFaddend The~strength of~the~anisotropy is governed by distortion parameter $\beta = f(z)/b(z)$, where $f(z)$ is the~logarithmic growth rate of~fluctuations \eqref{eq:grw_rate} and~$b$ is the~bias. By modeling the~full redshift-space galaxy power spectrum one can obtain a~combination of~the~product of~the~matter clustering amplitude and~the~growth rate.
...
\DIFdelbegin \DIFdel{The~key challenge in~modeling RSD is accounting for~non-linear effects, including non-linear or scale-dependent bias between galaxies and}\DIFdelend \DIFaddbegin } 
\\\textbf{\DIFadd{Cosmological Surveys}}
\DIFadd{There are many projects and~missions which study properties of~the~dark energy, either as a~main scientific goal or as a~complementary program. Here we mention some of the biggest surveys while directing the reader for further references.
}\\\textbf{\DIFadd{Sloan Digital Sky Surveys}}
\DIFadd{Sloan Digital Sky Surveys (SDSS, \mbox{%DIFAUXCMD
\cite{SDSS}}\hspace{0pt}%DIFAUXCMD
) aims to~create the~most detailed three-dimensional maps of~the~Universe. From the~beginning of~regular surveys in~2000 till 2014 there were seven finished surveys in~total (SDSS-I/II results \mbox{%DIFAUXCMD
\cite{SDSS_I_II}}\hspace{0pt}%DIFAUXCMD
, SDSS-III results \mbox{%DIFAUXCMD
\cite{BOSS_results}}\hspace{0pt}%DIFAUXCMD
), while there are three ongoing surveys from 2014 (SDSS-IV, \mbox{%DIFAUXCMD
\cite{2017AJ....154...28B}}\hspace{0pt}%DIFAUXCMD
) and~a~planned panoptic spectroscopic survey (SDSS-V, \mbox{%DIFAUXCMD
\cite{2017arXiv171103234K}}\hspace{0pt}%DIFAUXCMD
) which will start collecting data in~summer 2020. The~Baryon Oscillation Spectroscopic Survey (BOSS, \mbox{%DIFAUXCMD
\cite{BOSS}}\hspace{0pt}%DIFAUXCMD
) was a~six-year program (Fall 2009 -- Spring 2014) designed to~measure the~scale of~baryonic acoustic oscillations (BAO, see }\autoref{sec:bao}\DIFadd{) in~the~clustering of~matter. The~Extended Baryon Oscillation Spectroscopic Survey (eBOSS, \mbox{%DIFAUXCMD
\cite{2016AJ....151...44D}}\hspace{0pt}%DIFAUXCMD
) is the~new cosmological survey within a~SDSS-IV six-year program. eBOSS conducts observations of~galaxies and quasars and will expand the~selection of~luminous red galaxies beyond that probed by BOSS.
}\\\textbf{\DIFadd{Dark Energy Survey}}
\DIFadd{The~Dark Energy Survey (DES) is a program designed to~uncover the~nature of~dark energy by measuring the history of~cosmic expansion with~high precision. DES is an~optical near-infrared survey of~5000 deg\sq\ of~the~South Galactic Cap. Starting in~August of~2013 and~continuing till January 2019, DES begun to~survey a~large swath of~the~southern sky out to~vast distances in~order to~provide new clues to~these most fundamental questions }\parencite{DES}\DIFadd{.
}\\\textbf{\DIFadd{Euclid}}
\DIFadd{Euclid is an~ESA (European Space Agency) high-precision space mission designed to study dark matter and dark energy through mapping the~geometry and~evolution of~the~Universe }\parencite{euclid,2010arXiv1001.0061R}\DIFadd{. For this purpose, the~Euclid will mainly use two independent cosmological probes -- weak gravitational lensing and~baryonic acoustic oscillation -- out to~redshift $z\sim2$. As complementary probes, Euclid will use galaxy clusters and}\DIFaddend ~\DIFdelbegin \DIFdel{matter, at}\DIFdelend \DIFaddbegin \DIFadd{the}\DIFaddend ~\DIFaddbegin \DIFadd{Integrated Sachs--Wolfe effect. The Euclid mission will start in~mid-2022. The~overview of~}\DIFaddend the~\DIFdelbegin \DIFdel{level of~accuracy demanded by the~LSST's precision}\DIFdelend \DIFaddbegin \DIFadd{Euclid system design and~scientific requirements can be found in~\mbox{%DIFAUXCMD
\cite{2011arXiv1110.3193L}}\hspace{0pt}%DIFAUXCMD
.
}\\\textbf{\DIFadd{Vera C. Rubin Observatory}}
\DIFadd{The~Vera C. Rubin Observatory project, previously known as the~Large Synoptic Survey Telescope, will conduct the~10-year Legacy Survey of~Space and~Time (LSST, \mbox{%DIFAUXCMD
\cite{lsst}}\hspace{0pt}%DIFAUXCMD
). LSST is a~ground-based telescope in Chile and will produce a~6-band (300 -- 1100 nm) wide-field deep astronomical survey over 20,000 deg\sq\ of~the~southern sky. The LSST will scan the sky very rapidly (more than 800 images each night) and each patch of~the~sky will be visited about 1000 times during the whole survey. LSST`s data will be used for studying the dark matter and~the~dark energy. LSST will also detect and~track potentially hazardous asteroids. The~project is in~the~construction phase and~will begin its full science operations in~2022 }\parencite{lsst_web}\DIFadd{.
}\\\textbf{\DIFadd{Planck}}
\DIFadd{The~Planck mission (\mbox{%DIFAUXCMD
\cite{planck}}\hspace{0pt}%DIFAUXCMD
) was a~European Space Agency mission with~significant participation from NASA. It was launched into space on~May 14, 2009, and~was orbiting the~second Lagrange point of~our Earth-sun system, about 1.5 million km (930,000 miles) away. Planck was measuring the~Cosmic Microwave Background (CMB) over a~broad range of~far-infrared wavelengths. The mission`s goal was to study the~geometry and~contents of~the~Universe }\parencite[for results, see][]{planck_cosm}\DIFadd{. Planck`s mission ended on~23 October 2013, after nearly 4.5 years of operations }\parencite{planck_web}\DIFadd{.
}\\\textbf{\DIFadd{Nancy Grace Roman Space Telescope}}
\DIFadd{The~Nancy Grace Roman Space Telescope (formerly known as WFIRST, the~Wide Field Infrared Survey Telescope) is a~NASA space mission designed to~study dark energy, exoplanets, and~infrared astrophysics }\parencite{wmap_web}\DIFadd{. It will perform wide-field imaging and spectroscopic survey of~the~near-infrared sky. These data will be used to determine the~expansion history of~the~Universe and~the~growth history of~larges-scale structures. WFIRST`s mission should start in~the~mid-2020s }\parencite{WFIRST_report}\DIFaddend .
 \clearpage{}
...
\DIFaddbegin \DIFadd{Some review parts of this chapter has already been studied and described in author`s work \textcite{mastersthesis_vrastil}. In this work, we deal with the~same topic of the $f(R)$ gravity. We also use various notations and equations we already defined in~the~former work and we need them here, such as the~chameleon field $\chi$ and related equations. As we further develop the original study of the chameleon, we also took over some parts of the text (written in italics). We use the text in sections }\hyperref[sec:fR]{\fR\ gravity}\DIFadd{, }\hyperref[sec_cham]{Chameleon Gravity}\DIFadd{, and }\hyperref[sec:other]{Other theories}\DIFadd{, where a general overview of~other~interesting theories is presented. In section }\hyperref[sec_cham]{Chameleon Gravity} \DIFadd{we also extended our previous analysis of chameleon field on cosmological scales and we also extended the study of the chameleon in spherical systems on the scales of galaxy clusters.
}

\DIFaddend \section{Standard $\Lambda$CDM model -- successes and~issues}
The~standard cosmological model, the~\LCDM\ model or the~concordance model, assumes that the~Universe \DIFdelbegin \DIFdel{was created }\DIFdelend \DIFaddbegin \DIFadd{originated }\DIFaddend in~the~Big Bang from infinitely hot and dense energy, and~now is the~Universe composed of~about 5\% ordinary matter, 27\% dark matter, and~68\% dark energy \parencite{redefineLCDM}. The~\LCDM\ model \DIFdelbegin \DIFdel{is based upon }\DIFdelend \DIFaddbegin \DIFadd{depends on }\DIFaddend two theoretical models -- the~Standard Model of~Particle Physics and~ the~General Theory of~Relativity. The~model also assumes that the~universe is homogeneous and~isotropic on~sufficiently large (cosmic) scales. The~model is mathematically described above in~\autoref{chpt:cosmo_evol}.

The~\LCDM\ model represents \DIFdelbegin \DIFdel{general relativity }\DIFdelend \DIFaddbegin \DIFadd{the Universe }\DIFaddend with~a~cosmological constant \(\Lambda\) \DIFdelbegin \DIFdel{which is }\DIFdelend \DIFaddbegin \DIFadd{described by general relativity. This cosmological constant is now }\DIFaddend associated with~dark energy\DIFdelbegin \DIFdel{and~a~universe containing }\DIFdelend \DIFaddbegin \DIFadd{. The other requirement of this model is the presence of }\DIFaddend sufficiently massive dark matter particles \DIFdelbegin \DIFdel{, i.e., }\DIFdelend \DIFaddbegin \DIFadd{-- the }\DIFaddend cold dark matter. However, the~nature of~both dark energy and~dark matter is unknown.
...
The~other \DIFdelbegin \DIFdel{great }\DIFdelend success of~the~Big bang model is the~prediction of~the~Cosmic Microwave Background (CMB) radiation and~its temperature. \DIFdelbegin \DIFdel{In~1948, \textcite{1948Natur.162..774A} calculated the~present }\DIFdelend \DIFaddbegin \DIFadd{\textcite{1948Natur.162..774A} predicted the~}\DIFaddend temperature of~the~CMB to~be \DIFdelbegin \DIFdel{about }\DIFdelend \DIFaddbegin \DIFadd{approximately }\DIFaddend 5 K, \DIFdelbegin \DIFdel{remarkably }\DIFdelend \DIFaddbegin \DIFadd{very }\DIFaddend close to~the~\DIFdelbegin \DIFdel{modern }\DIFdelend \DIFaddbegin \DIFadd{current }\DIFaddend value of~\DIFdelbegin \DIFdel{about }\DIFdelend 2.73 K, determined by the~COBE satellite. Besides, the~COBE results showed \DIFdelbegin \DIFdel{an~}\DIFdelend \DIFaddbegin \DIFadd{that the CMB is }\DIFaddend extremely isotropic and \DIFdelbegin \DIFdel{~homogeneousCMB. This led to ~the~need for~an~inflationary phase }%DIFDELCMD < \parencite{1981PhRvD..23..347G} %%%
\DIFdel{of~}\DIFdelend \DIFaddbegin \DIFadd{homogeneous. These smooth properties of the CMB are not easily explainable in the original Big Bang model and they have led to the~theory of~inflation }\parencite{1981PhRvD..23..347G}\DIFadd{. This model predicts }\DIFaddend strongly accelerated expansion \DIFdelbegin \DIFdel{prior to~the~decoupling of ~photons from ordinary matter}\DIFdelend \DIFaddbegin \DIFadd{of the Universe during its very early stage}\DIFaddend .

The~\LCDM\ model has an~additional major assumption about the~existence of~dark matter. The \DIFdelbegin \DIFdel{~notion }\DIFdelend \DIFaddbegin \DIFadd{original need }\DIFaddend of~dark matter \DIFdelbegin \DIFdel{arose }\DIFdelend \DIFaddbegin \DIFadd{came }\DIFaddend from observations of \DIFdelbegin \DIFdel{~large astronomical objects such as galaxies and~clusters of~galaxies, which displayed gravitational effects that could not be accounted for~by the~visible matter. In~particular, the~}\DIFdelend \DIFaddbegin \DIFadd{galaxy clusters }\parencite{zwicky} \DIFadd{and, later, from }\DIFaddend observations of~\DIFdelbegin \DIFdel{\textcite{1980ApJ...238..471R}, who measured the~rotation curves for~the~luminous matter of~many spiral galaxies together with~the~observations of~\textcite{1978PhDT.......195B}, who compiled }\DIFdelend \DIFaddbegin \DIFadd{galaxy rotation curves }\parencite{1980ApJ...238..471R} \DIFadd{and }\DIFaddend 21 cm rotation curves for~neutral hydrogen gas \DIFdelbegin \DIFdel{that extended far beyond the~luminous matterof~each galaxy, showed that the~composite rotation curves were essentially flat out to~the~edge of~the~21 cm data. This implied that considerably more mass was required to ~be present in~each galaxy. This invisible }\DIFdelend \DIFaddbegin \parencite{1978PhDT.......195B}\DIFadd{. All these objects moved under gravitational forces that could not be caused only by the~visible matter. Considerably more gravitational mass was needed to explain these anomalous movements. This missing }\DIFaddend matter was called dark matter\DIFdelbegin \DIFdel{and~since to~date no }\DIFdelend \DIFaddbegin \DIFadd{. Up-to-date, no experiment searching for direct evidence of the }\DIFaddend dark matter has \DIFdelbegin \DIFdel{been definitely detected and ~the~nature of~dark matter remains unknown}\DIFdelend \DIFaddbegin \DIFadd{confirmed its existence and its nature remains unexplained}\DIFaddend .

\DIFdelbegin \DIFdel{The~notion of~dark energy arose from two }\DIFdelend \DIFaddbegin \DIFadd{Two }\DIFaddend sets of~observations of~supernovae of~Type Ia by \textcite{riess} and~\textcite{1999ApJ...517..565P} \DIFdelbegin \DIFdel{that }\DIFdelend suggested that the~expansion of~the~universe is accelerating. \DIFdelbegin \DIFdel{The~conclusion from these observations was that the ~universe had to~contain enough energy to ~overcome }\DIFdelend \DIFaddbegin \DIFadd{As standard forms of matter cause only attractive gravitational forces, which decelerate the expansion, some new kind of matter-energy is needed to overcome the }\DIFaddend gravity. This \DIFaddbegin \DIFadd{mysterious }\DIFaddend energy was named \DIFdelbegin \DIFdel{“}\DIFdelend dark energy.
\DIFdelbegin \DIFdel{”
}\DIFdelend 
...
where $\Mpl\equiv(\sqrt{8\pi G})^{-1}$ is the~reduced Planck mass, $R$ is the~Ricci scalar (curvature), $\Lambda_B$ the~bare cosmological constant and~$S_m[\psi_m;g_\uv]$ represents the~action of~matter fields. The~first term $(R)$ is the~standard Einstein-Hilbert action. The~\DIFaddbegin \DIFadd{second term consists of the~}\DIFaddend \textit{bare} \DIFdelbegin \DIFdel{cosmological constant appears in~the~second term of~the~above expression }\DIFdelend \DIFaddbegin \DIFadd{cosmological constant }\DIFaddend and~it is \DIFdelbegin \DIFdel{merely }\DIFdelend \DIFaddbegin \DIFadd{just }\DIFaddend a~new parameter of~the~total action. \DIFdelbegin \DIFdel{As it }\DIFdelend \DIFaddbegin \DIFadd{The cosmology constant in the action above }\DIFaddend is compatible with~\DIFdelbegin \DIFdel{general covariance this term appears to~be }\DIFdelend \DIFaddbegin \DIFadd{the~principle of~general covariance and it is }\DIFaddend natural from the~relativistic point of \DIFdelbegin \DIFdel{~}\DIFdelend view. The~third term in~the~\DIFdelbegin \DIFdel{above equation }\DIFdelend \DIFaddbegin \DIFadd{action }\DIFaddend denotes the~generic matter action. Variation of~the~\DIFdelbegin \DIFdel{total action }\DIFdelend \DIFaddbegin \DIFadd{action }\eqref{eq:S_GR} \DIFaddend with~respect to~the~metric tensor \DIFaddbegin \DIFadd{$g\uv$ }\DIFaddend leads to~the~Einstein equations
\DIFdelbegin \DIFdel{of~motion
}\DIFdelend \eq{
...
As shown by \textcite{1968SPhD...12.1040S}, \DIFdelbegin \DIFdel{when one takes into account }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend quantum field theory \DIFdelbegin \DIFdel{the~picture is changed}\DIFdelend \DIFaddbegin \DIFadd{changes the~situation}\DIFaddend . The~stress-energy tensor of~a~field placed in~the~vacuum \DIFdelbegin \DIFdel{state }\DIFdelend is given by
\eq{
    \left\langle0\left|T_\uv\right|0\right\rangle=-\rho\vac g_\uv\,,
}
where $\rho\vac$ is the~constant energy density of~the~vacuum. The~vacuum fluctuations are\DIFdelbegin \DIFdel{just a~specific type }\DIFdelend \DIFaddbegin \DIFadd{, therefore, just another form }\DIFaddend of~energy and \DIFdelbegin \DIFdel{, in~general relativity, all forms of~energy gravitate. Therefore}\DIFdelend \DIFaddbegin \DIFadd{as such, they must gravitate. If we take them into account}\DIFaddend ,  the~Einstein equations \DIFdelbegin \DIFdel{when quantum field theory is taken into account are
}\DIFdelend \DIFaddbegin \DIFadd{read
}\DIFaddend \eq{
...
\DIFdelbegin \DIFdel{Therefore, we conclude that the}\DIFdelend \DIFaddbegin \DIFadd{The}\DIFaddend ~effective cosmological constant \DIFaddbegin \DIFadd{$\Lambda\eff$ }\DIFaddend is the~sum of the~bare cosmological constant and~a~contribution \DIFdelbegin \DIFdel{originating }\DIFdelend from the~vacuum fluctuations. \DIFdelbegin \DIFdel{The~}\DIFdelend \DIFaddbegin \DIFadd{When making observations using the~Einstein equations, it is this }\DIFaddend effective cosmological constant \DIFdelbegin \DIFdel{$\Lambda\eff$ is the~quantity that one can observe and~constrain when tests of~the ~Einstein equations are carried out. The}\DIFdelend \DIFaddbegin \DIFadd{that governs the results. Another}\DIFaddend ~problem is that $\rho\vac$ \DIFdelbegin \DIFdel{is made of~several terms which are all }\DIFdelend \DIFaddbegin \DIFadd{includes several different terms, each of them }\DIFaddend huge in~comparison with~the~observed value of~$\Lambda\eff$ and~need to~be fine-tuned.
...
The~one-loop contributions \eqref{eq:vac_all} to~the~vacuum energy can be fine-tuned via the~bare cosmological constant and~associated $\rho_B$. Even though the~cancellation has to~be very precise (one part in~$\sim10^{60}$) we can be fine with~this solution. Problems come with~two-loops correction which is not significantly suppressed with~respect to~the~one-loop contribution \parencite{2012CRPhy..13..566M}. \DIFdelbegin \DIFdel{Therefore, the}\DIFdelend \DIFaddbegin \DIFadd{The}\DIFaddend ~cancellation imposed at~one-loop is \DIFdelbegin \DIFdel{completely spoilt , }\DIFdelend \DIFaddbegin \DIFadd{spoilt }\DIFaddend and~one must retune the~finite contributions in~the~counterterm to~\DIFdelbegin \DIFdel{more or less the~same }\DIFdelend \DIFaddbegin \DIFadd{similar }\DIFaddend degree of~accuracy. If we go further, to~the~three loops, four loops, and~so on, we are required to~fine-tune each time to~extreme accuracy. This is radiative instability and~the~main cosmological constant problem.
...
\DIFdelend \DIFaddbegin \section[$f(R)$-gravity]{\DIFaddend $f(R)$-gravity\DIFaddbegin \footnote{\DIFadd{Some parts of this section have already been published in \textcite{mastersthesis_vrastil}. The overtaken text is written in italics.}}\DIFaddend }
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin } 
\DIFaddend \subsection{Jordan vs. Einstein Frame}
\DIFaddbegin {\itshape
\DIFaddend The~action \eqref{eq:S_fr} is described in~the~so-called Jordan frame, where the~matter fields are minimally coupled to~the~metric and~follow geodesics. We can also describe the~action in~the~so-called Einstein frame, where ``standard'' gravity is restored. Using the~conformal transformations
\eq{
\label{eins_trans}
\begin{split}
...
\DIFdelbegin \DIFdel{Many contradictory arguments (sometimes incorrect) of~both sides result in~confusion and~ambiguous viewpoints.
}\DIFdelend 

\DIFdelbegin \DIFdel{It has been shown in~\textcite{Magnano:1993bd} that }\DIFdelend \DIFaddbegin \DIFadd{In~\textcite{Magnano:1993bd} proved the }\textit{\DIFadd{mathematical}} \DIFadd{equivalence }\DIFaddend these two frames \DIFdelbegin \DIFdel{are }\textit{\DIFdel{mathematically}} %DIFAUXCMD
\DIFdel{equivalent}\DIFdelend , i.e. every solution in~one frame implies an~existence of~a~solution in~the~other frame and~can be mapped into this frame. The~confusion about their physical equivalence comes from interpretations of~experiment results. For~example, \textit{ordinary} cosmology described by Einstein’s theory leads to~an~expanding universe solution. \DIFdelbegin \DIFdel{Coming }\DIFdelend \DIFaddbegin \DIFadd{However, if we transform the equations }\DIFaddend to~the~Jordan frame\DIFdelbegin \DIFdel{we can interpret }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend the~scale factor \DIFaddbegin \DIFadd{can be interpreted }\DIFaddend as a~scalar field which is coupled to~matter. \DIFdelbegin \DIFdel{In~this case, the}\DIFdelend \DIFaddbegin \DIFadd{The}\DIFaddend ~redshift of~spectral lines is \DIFdelbegin \DIFdel{no longer interpreted as an~effect due to~the~expansion of ~the~Universe, but due to~}\DIFdelend \DIFaddbegin \DIFadd{now seen as a result of }\DIFaddend growth of~coupling constants \DIFdelbegin \DIFdel{such that the~present transition energies are higher than those }\DIFdelend \DIFaddbegin \DIFadd{which makes transition energies higher than }\DIFaddend in~the~past \DIFdelbegin \DIFdel{. Hence the ~Jordan frame physicist does not see an~expanding Universe, but growing coupling constants}\DIFdelend \DIFaddbegin \DIFadd{and not the expansion of~the~Universe}\DIFaddend . Nevertheless, the~measured redshift \DIFdelbegin \DIFdel{of~spectral lines }\DIFdelend is the~same for~both frames.

\DIFaddbegin {\itshape
\DIFaddend Both frames have some issues with~fundamental principles. In~the~Jordan frame, the~weak energy condition can be violated and~hence states with~the~negative energy are possible. Moreover, there is no guarantee of~stability of~the~ground state. All \textit{classical} fields are believed to~satisfy the~energy condition but no so in~quantum theories. On~the~other hand in~the~Einstein frame, the~weak energy condition is satisfied but due to~the~non-universal coupling of~the~matter fields, the~equivalence principle is violated. However, this violation is only weak and~can pass the~Solar system tests.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin }
\DIFaddend \subsection{Screening Mechanisms}
\DIFaddbegin {\itshape
\DIFaddend We know that general relativity with~the~cosmological constant and~assumptions about the~cold dark matter can describe our universe very well. That means that any modified cosmology must be able to~recover \LCDM\ cosmology to~high accuracy. This is not normally an~issue. However, since modifications of~GR typically involve an~extra scalar degree of~freedom there are interactions with~matter that are unavoidable -- no symmetry can prevent all couplings to~the~standard model. This coupling to~matter means that there should be a~fifth force. Because we do not see any fifth forces or modifications of~gravity in~the~laboratory or in~the~Solar System we need to~suppress these fifth forces -- we need some sort of~a~\textit{screening mechanism}.
...
\DIFdelbegin %DIFDELCMD < \section{%%%
\DIFdelend \DIFaddbegin } 
\section[Chameleon Gravity]{\DIFaddend Chameleon Gravity\DIFaddbegin \footnote{\DIFadd{Some parts of this section have already been published in \textcite{mastersthesis_vrastil}. The overtaken text is written in italics.}}\DIFaddend }
\label{sec_cham}
...
\DIFaddbegin {\itshape
\DIFaddend If the~couplings $\beta_i$ are the~same for~each matter component with~the~same $w$ (we can omit the~radiation in~the~sum) and~the~overall density is $\rho=\sum_i\rho_i$, then the~effective potential reads
\eq{
...
\DIFaddbegin }
\DIFaddend \subsection{Chameleon Force}
\DIFaddbegin {\itshape
\DIFaddend The~interaction of~the~chameleon field with~matter is described by the~conformal coupling \eqref{eins_trans}. Free matter fields $\psi_m^{(i)}$ follow geodesics of~the~Jordan frame metric. In~the~Einstein frame, they follow modified trajectories affected by the~chameleon field \parencite{Waterhouse:2006wv}
\eq{
...
\DIFaddbegin }
\DIFaddend \subsection{Chameleon mechanism}
\DIFaddbegin {\itshape
\DIFaddend As discussed previously, we need some sort of~a~screening mechanism to~avoid Solar System tests of~GR. It means as seen from \eqref{cham_force} that the~chameleon potential needs to~approach some constant value in~dense regions or at~least have a~marginally suppressed amplitude.
...
\DIFaddbegin } 
\DIFaddend \subsection{Chameleon Profile}
\DIFaddbegin {\itshape
\DIFaddend \label{cham_prof}
...
\DIFaddbegin } 
\DIFaddend \subsection{Hu-Sawicki \texorpdfstring{\textit{\lowercase{f}(R)}}{fR} Model}
...
\subsubsection{Stars}
\DIFaddbegin {\itshape
\DIFaddend We will first consider a~case where some approximate solutions exist -- a~compact spherical object of~constant density $\rho_c$ surrounded by the~background of~density $\rho_0$ as discuss in~\autoref{cham_prof}. We expect that for~low-mass objects the~chameleon field will track the~Newtonian potential while for~massive objects the~chameleon field will be frozen inside the~sphere and~outside it will be following the~Newtonian behavior but with~decreased amplitude.
\DIFaddbegin }
\DIFaddend 
...
\DIFaddbegin {\itshape
\DIFaddend Let us focus on~the~regime which cannot be treated analytical, i.e. regime between thin-shell and~thick-shell solutions. This regime corresponds to~the~situation when the~linear approximation (thick-shell) breaks down inside the~object, i.e. the~gravitational potential cancels screening potential somewhere inside the~object. We will denote $\Req$ the~\textit{equivalence radius} -- radius at~which the~Newtonian potential equals the~screening potential $|\Phi_G(\Req)|=\Phi_s$. By letting the~equivalence radius posses also negative values such as $(1+|\Req|/R_s)|\Phi_G(0)|=\Phi_s$ we can clearly distinguish between the~linear $(\Req<0)$ and~the~screening $(\Req>0)$ regime.\DIFaddbegin }
\DIFaddend 
...
\DIFaddbegin {\itshape
\DIFaddend The~Navarro-Frenk-White (NFW) profile proposed by \textcite{1996ApJ...462..563N} describes the~distribution of~cold dark matter. The~NFW profile of~matter overdensity is given by
\eq{
	\label{NFW_rho}
...
\end{figure*} 
\DIFdelbegin %DIFDELCMD < \section{%%%
\DIFdelend \DIFaddbegin \section[Other theories]{\DIFaddend Other theories\DIFaddbegin \footnote{\DIFadd{Some parts of this section have already been published in \textcite{mastersthesis_vrastil}. The overtaken text is written in italics.}}\DIFaddend }
...
The~inhomogeneous LTB model matches to \DIFdelbegin \DIFdel{~}\DIFdelend the~supernovae data and \DIFdelbegin \DIFdel{~}\DIFdelend the~location of~the~first acoustic peak of~the~CMB temperature power spectrum but cannot satisfactorily reproduce the~entire CMB angular power spectrum \parencite{2011JCAP...02..013C}. The~observed isotropy of~the~CMB radiation implies that we must live close to~the~center of~the~void -- nearer than 15 Mpc \parencite{2006PhRvD..74j3520A}. Moreover, there is no valid mechanism at~present to~explain the~formation of~such huge inhomogeneities, let alone one with~our Galaxy near the~center.
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin } 
\DIFaddend \section{Parametrization of~models}
...
The~leapfrog integrator \DIFdelbegin \DIFdel{is an~example of~a~symplectic integrator}\DIFdelend \DIFaddbegin \DIFadd{belongs to a~class of~symplectic integrators }\parencite{2011EPJP..126...55D}\DIFaddend . Symplectic integrators \DIFdelbegin \DIFdel{exactly solve an~approximate Hamiltonian. As a~consequence, the~numerical time evolution is a~canonical map and ~preserves certain conserved quantities exactly, }\DIFdelend \DIFaddbegin \DIFadd{are canoncial transformations and as such, they conserve certain quantities }\DIFaddend such as the~total angular momentum, the~phase-space volume, and~the~Jacobi constants. \DIFdelbegin \DIFdel{The~idea is to~approximate the~Hamiltonian \(H\) governing motion of~particles with~an~approximate one
}\DIFdelend \DIFaddbegin \DIFadd{Instead of solving the~exact Hamiltonian approximately, the~leapfrog integrator tries to solve an approximate Hamiltonian exactly
}\DIFaddend \eq{
    \tilde H=H+H\err\,,
}
where \(H\err\) is the~error Hamiltonian. \DIFdelbegin \DIFdel{Provided that }\DIFdelend \DIFaddbegin \DIFadd{If }\DIFaddend \(\tilde H\) and~\(H\) \DIFdelbegin \DIFdel{are time-invariant}\DIFdelend \DIFaddbegin \DIFadd{do not dependent explicitly on time}\DIFaddend , the~\DIFdelbegin \DIFdel{energy error }\DIFdelend \DIFaddbegin \DIFadd{error in~energy }\DIFaddend is bounded at~all times. \DIFdelbegin \DIFdel{The~goal now is to ~find }\DIFdelend \DIFaddbegin \DIFadd{We want to find such a }\DIFaddend \(\tilde H\) that can be solved \DIFdelbegin \DIFdel{exactly by simple numerical means }\DIFdelend \DIFaddbegin \DIFadd{easily by numerical methods and~simultaneously }\DIFaddend and~minimizes \(H\err\). \DIFdelbegin \DIFdel{Defining the~combined phase-space coordinates \(w\equiv(x,p)\) the}\DIFdelend \DIFaddbegin \DIFadd{The}\DIFaddend ~Hamilton's equations are
\eq{
    \dot w=\HH w\,,
}
where \DIFaddbegin \DIFadd{\(w\equiv(x,p)\) are combined phase-space coordinates and }\DIFaddend \(\HH\equiv\{\cdot,H\}\) is an~operator acting on~\(w\) through Poisson bracket. Hamilton’s equations have then the~formal solution
\eq{
    w(t+\Delta t)=e^{\Delta t\HH}w(t)\,.
}
\begin{sloppypar}
\DIFdelbegin \DIFdel{The~}\DIFdelend \DIFaddbegin \DIFadd{We want~to split the~}\DIFaddend operator \(e^{\Delta t\HH}\) \DIFdelbegin \DIFdel{can be split , in~an~approximate sense, into a~succession of~discrete but symplectic steps , each of~which can be exactly integrated . The~most common choice is to~separate out }\DIFdelend \DIFaddbegin \DIFadd{(approximately) into steps that, individually, can be integrated exactly. This is usually done by separating the Hamiltonian into }\DIFaddend the~kinetic and~potential energies, \({H=T(p)+V(x)}\)\DIFdelbegin \DIFdel{, such that we can split
}\DIFdelend \DIFaddbegin \DIFadd{. That we can write
...
\label{sec:PM}
As we mentioned in~the~introduction, in~order to~effectively compute forces on~particles one has to~abandon the~continuous position and~solve the~forces using discretized positions. The~idea of~the~PM method is that we set up a~mesh (grid) over the~computational box, and~then \DIFaddbegin \DIFadd{we }\DIFaddend solve the~gravitational potential (i.e. Poisson’s equation \eqref{eq:poisson_lin}) at~the~mesh-points \DIFaddbegin \parencite{n_body_Lindholm}\DIFaddend . Forces at~the~mesh-points can then be obtained by calculating the~gradient of~the~potential. The~four principal steps of~the~particle-mesh calculation are \DIFaddbegin \parencite{Hockney:1988:CSU:62815}\DIFaddend :
\begin{enumerate}
...
In~step \ref{it:pm_1} and~\ref{it:pm_4} we make a~connection between discrete mesh and~continuous positions of~the~particles. In~the~first, we need to~distribute the~mass of~all particles onto the~mesh and~in~the~last step, on~the~contrary, we need to~interpolate forces known on~the~discrete mesh onto particles with~continuous positions. \DIFdelbegin \DIFdel{There are several ways of~assigning the~particle masses }\DIFdelend \DIFaddbegin \DIFadd{We can assign the~masses of particles }\DIFaddend to~the~discrete density function $\delta$ and~\DIFaddbegin \DIFadd{the~}\DIFaddend discrete force $-\nabla\Phi$ to~the~particles \DIFaddbegin \DIFadd{in several ways}\DIFaddend :
...
\DIFdelbegin \DIFdel{The~essence of~the ~method is to~express }\DIFdelend \DIFaddbegin \DIFadd{In the P$^3$M algorithm, one writes }\DIFaddend the~interparticle force as the~sum of~\DIFdelbegin \DIFdel{two parts; }\DIFdelend the~short-range \DIFdelbegin \DIFdel{part}\DIFdelend \DIFaddbegin \DIFadd{force}\DIFaddend , which is \DIFdelbegin \DIFdel{nonzero only for~particle separations less than }\DIFdelend \DIFaddbegin \DIFadd{limited by }\DIFaddend some cutoff radius,and~the~\DIFdelbegin \DIFdel{smoothly varying long-range part, which has a~transform which }\DIFdelend \DIFaddbegin \DIFadd{long range force, which varies smoothly and~}\DIFaddend is approximately band-limited (that is to~say, is approximately nonzero for~only a~limited range of~$k$). The~\DIFdelbegin \DIFdel{total }\DIFdelend short-range force on~a~particle is computed by direct particle-particle (PP) \DIFdelbegin \DIFdel{pair }\DIFdelend force summation and~the~\DIFdelbegin \DIFdel{smoothly varying part is approximated by }\DIFdelend \DIFaddbegin \DIFadd{longe-range force is computed using }\DIFaddend the~particle-mesh (PM) \DIFdelbegin \DIFdel{force calculation}\DIFdelend \DIFaddbegin \DIFadd{algorithm}\DIFaddend .
...
Practical multi-grid techniques were introduced by \textcite{10.2307/2006422}. These methods also \DIFdelbegin \DIFdel{interpolate the~density and~potential between grid and ~particles }\DIFdelend \DIFaddbegin \DIFadd{use the~discrete grids and continuous particles positions }\DIFaddend as PM methods based on~FFT with~$\OO(N_m\log N_m)$ complexity but they use relaxation methods such as Gauss-Seidel iteration \parencite{doi:10.1002/zamm.19720520813} and~can solve elliptic partial differential equations using $\OO(N_m)$ operations. The~\DIFdelbegin \DIFdel{numerical coefficients in~these estimates are such that multigrid methods are comparable to ~the~rapid methods in~execution speed. Unlike the ~}\DIFdelend \DIFaddbegin \DIFadd{speed of~these methods is comparable to the PM and other }\DIFaddend rapid methods, \DIFdelbegin \DIFdel{however, }\DIFdelend \DIFaddbegin \DIFadd{but }\DIFaddend the~multigrid methods can \DIFaddbegin \DIFadd{also }\DIFaddend solve general elliptic equations with~nonconstant coefficients\DIFdelbegin \DIFdel{with~hardly any loss of~efficiency}\DIFdelend . Even non-linear equations of~modified gravitational can be solved with~these methods \parencite{10.5555/42249}.

The~basic \DIFdelbegin \DIFdel{idea exploits }\DIFdelend \DIFaddbegin \DIFadd{principle of the multigrid methods is based on }\DIFaddend the~fact\DIFdelbegin \DIFdel{that on ~a~coarser }\DIFdelend \DIFaddbegin \DIFadd{, than on a~smaller (coarser) grid, we can achieve an approximate solution with small long-range errors faster than on~a~bigger (finer) }\DIFaddend grid\DIFdelbegin \DIFdel{relaxation occurs faster because information travels faster . The~distribution of~errors (the~difference between the~actual density -- source -- and~that obtained via the~discretization of~potential from the~current estimate for~the~potential) is first smoothed on~the~finest gridby a~few }\DIFdelend \DIFaddbegin \DIFadd{. The~problem is sequentially transferred to~coarser and~coarser grids, each time smoothing the~errors via a~few }\DIFaddend Gauss-Seidel \DIFdelbegin \DIFdel{iterations. After transferring the~problem to~a~coarser grid, the~process is repeated on~coarser and~coarser grids until}\DIFdelend \DIFaddbegin \DIFadd{iterations, until~the convergence }\DIFaddend on~the~coarsest grid \DIFdelbegin \DIFdel{convergence }\DIFdelend is achieved. \DIFdelbegin \DIFdel{Then the}\DIFdelend \DIFaddbegin \DIFadd{The}\DIFaddend ~problem is \DIFaddbegin \DIFadd{then }\DIFaddend transferred back to~finer and~finer grids, each time iterating until convergence \DIFaddbegin \parencite{2011EPJP..126...55D}\DIFaddend .
\subsection{Simple two-grid V-cycle}
\DIFdelbegin \DIFdel{The~key idea of~the~multigrid method can be understood by considering the~simplest }\DIFdelend \DIFaddbegin \DIFadd{Let us first consider the~simple }\DIFaddend case of~a~two-grid method. \DIFdelbegin \DIFdel{Suppose we are trying }\DIFdelend \DIFaddbegin \DIFadd{We want }\DIFaddend to~solve the~discretized linear elliptic problem on~a~uniform grid with~mesh size $h$
\eq{
    \LL_h u_h=f_h\,,
}
...
To~find $v_h$ we need to~make an~approximation to~$\LL_h$. This can be done using classical iteration methods, such as Jacobi or Gauss-Seidel, which use a~simpler operator than $\LL_h$, e.g. only the~diagonal part \DIFdelbegin \DIFdel{(Jacobi iteration) }\DIFdelend \DIFaddbegin \DIFadd{for~the~Jacobi iteration, }\DIFaddend or the~lower triangle \DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{for~the~}\DIFaddend Gauss-Seidel iteration\DIFdelbegin \DIFdel{)}\DIFdelend .
...
\end{itemize}
To~achieve the~full multigrid method is now straightforward. Instead of~solving the~coarse-grid defect exactly, we can get an~approximate solution \DIFdelbegin \DIFdel{of~it }\DIFdelend by introducing an~even coarser grid and~so on.
...
\subsection{TreePM}
The~tree code was pioneered by \textcite{1986Natur.324..446B} and~uses a~hierarchical spatial tree to~define localized groups of~particles. This has an~advantage over an~equidistant mesh when dealing with~highly inhomogeneous systems like stellar dynamics when few cells \DIFdelbegin \DIFdel{would }\DIFdelend \DIFaddbegin \DIFadd{can }\DIFaddend contain most of~the~particles \DIFdelbegin \DIFdel{. With~the~usual }\DIFdelend \DIFaddbegin \DIFadd{in the system. When using the~}\DIFaddend oct-tree, each cubic cell containing more than some maximum number of~particles is split into eight child cells of~half their parent's size. \DIFdelbegin \DIFdel{This results in~}\DIFdelend \DIFaddbegin \DIFadd{In~this way, }\DIFaddend a~tree-like hierarchy of~cubic nodes \DIFdelbegin \DIFdel{with~the~root box, containing all particles, at~its bottom}\DIFdelend \DIFaddbegin \DIFadd{is created}\DIFaddend . The~particles within each of~the~tree nodes constitute a~well-defined and~localized group. The~potential of~a~cell is obtained by multipole expansion around some center of~the~group of~particles (usually center of~mass). The~gravity at~any point is then approximated by the~summation of~potentials from all other cells while considering only cells with~a~sufficiently small opening angle (size of~the~cell over the~distance). If the~opening angle is big the~same algorithm is applied to~the~daughter cells \DIFaddbegin \parencite{2011EPJP..126...55D}\DIFaddend .
\subsection{Fast multipole methods}
The~fast multipole method of~\textcite{1987JCoPh..73..325G} also works with~localized particle groups and~was originally proposed using a~fixed grid but \textcite{2000ApJ...536L..39D} implemented the~method using a~tree structure with~better results. The~method works similarly as the~tree code but in~addition to~expanding the~potential at~the~source positions, it also expands it at~the~sink positions. This \DIFdelbegin \DIFdel{dual expansion at~both ends of~all interactions considerably }\DIFdelend speeds up the~\DIFdelbegin \DIFdel{simultaneous }\DIFdelend computation of~gravity for~all particles \DIFdelbegin \DIFdel{but brings }\DIFdelend \DIFaddbegin \DIFadd{significantly but has }\DIFaddend no advantage over the~tree \DIFdelbegin \DIFdel{code }\DIFdelend \DIFaddbegin \DIFadd{methods }\DIFaddend when computing the~force at~a~single position \DIFaddbegin \parencite{2011EPJP..126...55D}\DIFaddend .

The~second part \DIFaddbegin \DIFadd{of the force calculation }\DIFaddend is the~interaction phase when the~coefficients of~the~Taylor series are evaluated for~each cell. If the~opening angle for~a~mutual cell-cell interaction is small enough the~coefficients for~the~interactions in~each direction are computed.  Otherwise, the~interaction is split into up to~8 new interactions by opening the~bigger of~the~two cells. The~total \DIFdelbegin \DIFdel{computational
costs of ~this algorithm are dominated by}\DIFdelend \DIFaddbegin \DIFadd{complexity of }\DIFaddend the~\DIFdelbegin \DIFdel{interaction phase, which only requires }\DIFdelend \DIFaddbegin \DIFadd{algorithm is given by~}\DIFaddend $\OO(N_p)$ interactions for~the~computation of~all $N_p$ particle forces. This represents a~\DIFdelbegin \DIFdel{substantial }\DIFdelend \DIFaddbegin \DIFadd{great }\DIFaddend reduction from the~$\OO(N_p^2)$ for~direct summation and~can greatly improves simulations of~astrophysical problems with~inhomogeneous particle distributions.
%
\clearpage{}
...
It was proposed by \textcite{1974ApJ...189L..51P} that \DIFdelbegin \DIFdel{clustering in~the~very }\DIFdelend \DIFaddbegin \DIFadd{we can study the~behavior of~a~}\DIFaddend non-linear \DIFdelbegin \DIFdel{regime might be understood byassuming that regions of~}\DIFdelend \DIFaddbegin \DIFadd{clustering in~}\DIFaddend high-density \DIFdelbegin \DIFdel{contrast undergo }\DIFdelend \DIFaddbegin \DIFadd{regions by~assuming, that these regions have undergone }\DIFaddend virialization and~\DIFdelbegin \DIFdel{subsequently }\DIFdelend \DIFaddbegin \DIFadd{now }\DIFaddend maintain a~fixed proper density, hence stable clustering. The~correlation function \DIFdelbegin \DIFdel{for~a~population }\DIFdelend of~such systems would \DIFdelbegin \DIFdel{then simply }\DIFdelend evolve according to
\eq{
	\xi(r,a)\propto1/\bar\rho\propto a^3\,.
}
\textcite{1991ApJ...374L...1H} developed a~\DIFdelbegin \DIFdel{method for~interpolating between }\DIFdelend \DIFaddbegin \DIFadd{model which enabled interpolating between the~}\DIFaddend linear theory on~large scales and~the~non-linear predictions of~the~stable clustering \DIFdelbegin \DIFdel{hypothesis }\DIFdelend on~small scales. They showed that the~non-linear \DIFdelbegin \DIFdel{volume-averaged }\DIFdelend two-point correlation function could be parameterized by a~simple function of~the~linear correlation function
\eq{
...
In~LPT, this displacement vector field is expanded in~a~perturbation series in~the~linear growth function $D$ in~Fourier space. Density perturbations $\delta$ are described as a~function of~the~displacement vector through conservation of~mass.\DIFdelbegin \DIFdel{This Lagrangian picture is intrinsically non-linear in~the~density field, and~a~small perturbation in~Lagrangian fluid element paths carries a~considerable amount of~non-linear information about the~corresponding Eulerian density and~velocity fields.}\DIFdelend \clearpage{}
...
\clearpage{}\DIFdelbegin \textbf{\DIFdel{Cosmological Surveys}}
%DIFAUXCMD
\addtocounter{chapter}{-1}%DIFAUXCMD
%DIFDELCMD < \label{chpt:cosmo_surveys}
%DIFDELCMD < %%%
\DIFdel{There are many projects and~missions which study properties of~the~dark energy, either as a~main scientific goal or as a~complementary program. Among the~big surveys are e.g. Sloan Digital Sky Surveys (SDSS, \mbox{%DIFAUXCMD
\cite{SDSS}}\hspace{0pt}%DIFAUXCMD
) which aims to~create the~most detailed three-dimensional maps of~the~Universe. From the~beginning of~regular surveys in~2000 till 2014 there were seven finished surveys in~total (SDSS-I/II results \mbox{%DIFAUXCMD
\cite{SDSS_I_II}}\hspace{0pt}%DIFAUXCMD
, SDSS-III results \mbox{%DIFAUXCMD
\cite{BOSS_results}}\hspace{0pt}%DIFAUXCMD
), while there are three ongoing surveys from 2014 (SDSS-IV, \mbox{%DIFAUXCMD
...
\cite{2017arXiv171103234K}}\hspace{0pt}%DIFAUXCMD
) which will start collecting data in~summer 2020. Other surveys are e.g. Wilkinson Microwave Anisotropy Probe (WMAP, \mbox{%DIFAUXCMD
\cite{WMAP}}\hspace{0pt}%DIFAUXCMD
, results \mbox{%DIFAUXCMD
\cite{WMAP_results}}\hspace{0pt}%DIFAUXCMD
); Planck (\mbox{%DIFAUXCMD
\cite{planck}}\hspace{0pt}%DIFAUXCMD
, results \mbox{%DIFAUXCMD
\cite{planck_cosm}}\hspace{0pt}%DIFAUXCMD
). There are also several planned surveys almost ready to~start collecting data -- Euclid }%DIFDELCMD < \parencite{euclid}%%%
\DIFdel{, LSST }%DIFDELCMD < \parencite{lsst} %%%
\DIFdel{or WFIRST }%DIFDELCMD < \parencite{WFIRST_report}%%%
\DIFdel{. Some parts of~this chapter are based on~the~author's work \textcite{mastersthesis_vrastil}.
}\\\textbf{\DIFdel{BOSS}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdel{The~Sloan Digital Sky Survey's (SDSS-III) Baryon Oscillation Spectroscopic Survey (BOSS) is a~six-year program (Fall 2009 -- Spring 2014)  that uses the~wide-field 2.5-m telescope at~Apache Point Observatory, see }%DIFDELCMD < \autoref{fig:sdss_tele}%%%
\DIFdel{. The~BOSS is designed to~measure the~scale of~baryonic acoustic oscillations (BAO, see }%DIFDELCMD < \autoref{sec:bao}%%%
\DIFdel{) in~the~clustering of~matter over a~larger volume than the~combined efforts of~all previous spectroscopic surveys of~large-scale structures. BOSS uses 1.5 million luminous galaxies to~measure BAO to~redshifts $z<0.7$. Observations of~neutral hydrogen in~the~Ly$\alpha$ forest in~more than 150,000 quasar spectra constrain BAO over the~redshift range $2.15 < z < 3.5$ \mbox{%DIFAUXCMD
\cite{BOSS}}\hspace{0pt}%DIFAUXCMD
.
}%DIFDELCMD < \begin{figure}[tb]
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=0.55\textwidth]{cosmo_surveys/SDSS_telescope_new.jpg}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Sloan Foundation 2.5m Telescope. }\textit{\DIFdelFL{Credit:}} %DIFAUXCMD
%DIFDELCMD < \url{http://www.sdss3.org/}%%%
}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:sdss_tele}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{There are two double spectrographs, each covering the~wavelength range 361 nm -- 1014 nm with~resolution $R=\lambda/\Delta\lambda$ ranging from 1300 at~the~blue end to~2600 at~the~red end.  Both spectrographs have a~red channel with~a~4k $\times$ 4k, 15}%DIFDELCMD < \um  %%%
\DIFdel{pixel CCD from Lawrence Berkeley National Laboratory (LBNL). Both spectrographs have a~blue channel with~a~4k $\times$ 4k, 15}%DIFDELCMD < \um  %%%
\DIFdel{pixel CCD from  e2v. The~instrument is fed by 1000 optical fibers (500 per spectrograph), each subtending 2\arcsec\ on~the~sky.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Using the~acoustic scale as a~physically calibrated ruler, BOSS determines the~angular diameter distance with~a~precision of~1\% at~redshifts $z = 0.3$ and~$z = 0.57$ using the~distribution of~galaxies and~measurements of~$H(z)$ to~1.8\% and~1.7\%  at~the~same redshifts. At~redshifts $z\sim2.5$ the~ angular diameter distance and~$H\mins(z)$ is measured to~an~accuracy of~1.9\% using Ly$\alpha$ forest.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{BAO measurements with~the~CMB-calibrated physical scale of~the~sound horizon and~SN data yields of~$H_0=(67.3\pm1.1)$ \unith\ with~1.7\% precision \mbox{%DIFAUXCMD
\cite{BOSS_results}}\hspace{0pt}%DIFAUXCMD
, see }%DIFDELCMD < \autoref{fig:boss_Hz}%%%
\DIFdel{. This measurement assumes standard pre-recombination physics but is insensitive to~assumptions about dark energy or space curvature. When we allow more general forms of~evolving dark energy, the~BAO+SN+CMB parameter constraints are always consistent with~flat \LCDM\ values at~$1\sigma$. While the~overall $\chi^2$ of~model fits is satisfactory, the~Ly$\alpha$ forest BAO measurements are in~moderate $(2-2.5\sigma)$ tension with~model predictions. Models with~early dark energy that tracks the~dominant energy component at~high redshift remain consistent with~expansion history constraints, and~they yield a~higher $H_0$ and~lower matter clustering amplitude, improving agreement with~some low-redshift observations.
}%DIFDELCMD < \begin{figure}[htbp]
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=0.7\textwidth]{cosmo_surveys/boss_Hz.png}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{BAO measurements and~model predictions of~\(H(z)\) and~\(D_M(z)\) as a~function of~redshift, with~physically informative scalings. The~top panel shows \(H(z)/(1+z)\), the~proper velocity between two objects 1 comoving Mpc apart. The~bottom panel shows \(c\ln{(1+z)}/D_M(z)\), a~scaling that matches a~constant line \(H(z)=(1+z)H_0\) in~the~top panel to~the~same constant line in~the~bottom panel for~a~flat universe. Filled circles and~squares show the~BOSS CMASS and~Ly$\alpha$ forest measurements of~\(H(z)\) and~\(D_M(z)\), respectively; we show the~Ly$\alpha$ forest quasar cross-correlation as crosses to~distinguish from the~Ly$\alpha$ forest auto-correlation measurements. Filled triangles in~the~bottom panel show the~BOSS LOWZ and~MGS measurements of~\(D_V (z)\) converted to~\(D_M(z)\). Open squares show the~value of~\(H_0 = 67.3\pm1.1\) \unith determined from the~combination of~BAO and~SN Ia data. The~grey swath in~both panels is the~prediction from the~Planck \LCDM\ cosmology including \(1\sigma\) parameter errors; in~the~top panel, one can easily see the~model transition from deceleration to~acceleration at~\(z\approx0.6\). The~dashed line shows the~\LCDM\ prediction using the~best-fit WMAP parameters, which has lower \(\Omega_mh^2\). Dotted curves show models that match the~best-fit Planck values of~\(\omega_{cb}\), \(\omega_{b}\), and~\(D_M(1090)/r_d\) but have \(\Omega_k = 0.01\) (blue), \(w = -0.7\) (green), or \(w = -1.3\) (red). The~}\textit{\DIFdelFL{x}}%DIFAUXCMD
\DIFdelFL{-axis is set to~\(\sqrt{1+z}\) both for~display purposes and~so that a~pure matter universe \((\Omega_m = 1)\) appears as a~decreasing straight line on~the~top panel. }\textit{\DIFdelFL{Note:}} %DIFAUXCMD
\DIFdelFL{Reprinted from \textcite{BOSS_results}.}}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:boss_Hz}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\\\textbf{\DIFdel{eBOSS}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdel{The~Extended Baryon Oscillation Spectroscopic Survey (eBOSS, \mbox{%DIFAUXCMD
\cite{2016AJ....151...44D}}\hspace{0pt}%DIFAUXCMD
) is the~new cosmological survey within a~SDSS-IV six-year program started in~2014 July. eBOSS will conduct novel cosmological observations of~galaxies, and~in~particular quasars, using the~same 1000-fiber optical spectrographs as those in~BOSS at~Apache Point Observatory. These observations will be conducted simultaneously with~the~Time Domain Spectroscopic Survey (TDSS, \mbox{%DIFAUXCMD
\cite{2015ApJ...806..244M}}\hspace{0pt}%DIFAUXCMD
) designed for~variability studies and~the~Spectroscopic Identification of~eROSITA Sources (SPIDERS, \mbox{%DIFAUXCMD
\cite{2010SPIE.7741E..1NF}}\hspace{0pt}%DIFAUXCMD
) program designed for~studies of~X-ray sources.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{eBOSS will map the~large-scale-structures over the~relatively unconstrained redshift range $0.6<z< 2.2$, see }%DIFDELCMD < \autoref{fig:eboss} %%%
\DIFdel{for~comparison with~the~BOSS range. eBOSS will expand the~selection of~luminous red galaxies (LRG) beyond that probed by BOSS and~obtain better than a~$1.0\%$ precision distance estimate when combined with~the~$z > 0.6$ tail of~the~BOSS galaxy population. With~observations of~a~new sample of~emission-line galaxies (ELG) eBOSS will produce a~$2.0\%$ precision distance estimate at~higher redshifts. eBOSS will obtain a~$1.8\%$ precision distance estimate in~the~redshift range $0.9 < z < 2.2$ using quasars that have luminosities and~areal densities well-suited to~the~sensitivity of~the~BOSS spectrographs. Finally, eBOSS will sharpen the~BOSS Ly$\alpha$ forest measurements by a~factor of~$1.44$ with~a~new selection of~$z > 2.1$ quasars, providing stronger leverage on~the~history of~dark energy.
}%DIFDELCMD < \begin{figure}[htb]
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=0.7\textwidth]{cosmo_surveys/pie_boss_eboss_marked.png}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Planned eBOSS coverage of~the~Universe. }\textit{\DIFdelFL{Credit:}} %DIFAUXCMD
%DIFDELCMD < \url{http://www.sdss.org/}%%%
}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:eboss}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{With~four classes of~spectroscopic targets (LRG, ELG, quasar, Ly$\alpha$ forest quasar), eBOSS will enable the~first high precision distance measurements in~the~epochs when dark energy emerged as the~dominant dynamical component of~the~Universe. In~addition to~BAO distance measurements, eBOSS will provide new tests of~GR on~cosmological scales through redshift-space distortions (RSD), new tests for~non-Gaussianity in~the~primordial density field, and~new constraints on~the~summed mass of~all neutrino species.
}\\\textbf{\DIFdel{DES}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{DES}
%DIFDELCMD < %%%
\DIFdel{The~Dark Energy Survey (DES) is designed to~probe the~origin of~the~accelerating universe and~to~help uncover the~nature of~dark energy by measuring the~\mbox{14-billion-year} history of~cosmic expansion with~high precision. DES is an~optical near-infrared survey of~5000 deg\sq\ of~the~South Galactic Cap up to~absolute magnitude $r\sim24$ in~the~$grizy$ spectrum. DES's instrument consists primarily of~a~new camera, Dark Energy Camera (DECam), specifically designed to~be sensitive to~the~highly redshifted light from distant galaxies. DECam is mounted on~a~classic telescope, the~Blanco 4-m telescope at~the~Cerro Tololo Inter-American Observatory (CTIO) in~La Serena, Chile. The~imaging system is supported by a~combination of~microwave and~optical data links that will provide the~recorded data to~the~survey members. Starting in~August of~2013 and~continuing for~five years, DES has begun to~survey a~large swath of~the~southern sky out to~vast distances in~order to~provide new clues to~these most fundamental questions \mbox{%DIFAUXCMD
\cite{DES}}\hspace{0pt}%DIFAUXCMD
.
}%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[htb]
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=0.7\textwidth]{cosmo_surveys/des.jpg}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{DECam operating at~night, while an~observer watches.}%DIFDELCMD < \\%%%
\textit{\DIFdelFL{Credit:}} %DIFAUXCMD
%DIFDELCMD < \url{https://www.darkenergysurvey.org/}%%%
}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:des}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdel{The~survey data allow to~measure the~dark energy and~dark matter densities and~the~dark energy equation of~state through four independent methods: galaxy clusters (counts and~spatial distributions at~$0.1<z<1.3$), weak gravitational lensing tomography (on~several redshift shells to~$z\sim1$), galaxy angular clustering, and~supernova distances (at~$0.3<z<0.8$).
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The~main tool is the~DECam, 74 2k $\times$ 4k 570 Mpx digital camera built at~Fermilab in~Batavia. It provides a~2.2\textdegree\ field of~view image at~0.27\arcsec/pixel. It covers the~wavelength range 400 -- 1100 nm with~five filters $(grizy)$. The~electronics will allow an~entire digital image to~be read out and~recorded in~17 seconds, time that it takes the~telescope to~move to~its next viewing position.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{From the~first two years of~observation, a~mass map from weak gravitational lensing shear measurements over 139 deg\sq\ has been reconstructed \mbox{%DIFAUXCMD
\cite{DES_mass}}\hspace{0pt}%DIFAUXCMD
. There is a~good agreement between the~mass map and~the~distribution of~massive galaxy clusters identified using a~red-sequence cluster finder. These measurements are consistent with~simulated galaxy catalogs based on~\LCDM\ \nbody\ simulations, suggesting low systematics uncertainties in~the~map.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In~\textcite{2019PhRvL.122q1301A} the~DES collaboration presented their cosmological constraints for~\wCDM: $\Omega_m=0.300^{+0.023}_{-0.021}$, $\Omega_b=0.064^{+0.013}_{-0.009}$, $\Omega_\Lambda=0.700^{+0.021}_{-0.023}$, $w=-0.80^{+0.09}_{-0.11}$ and~$\sigma_8=0.786^{+0.029}_{-0.019}$. Constraints on~the~dark energy equation of~state $w$ and~$\Omega_m$ for~\wCDM\ are also shown in~}%DIFDELCMD < \autoref{fig:des_res}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[htb]
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=0.55\textwidth]{cosmo_surveys/DES_constraints.png}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Constraints on~the~dark energy equation of~state $w$ and~$\Omega_m$ in~a~\wCDM\ model with~fixed curvature $(\Omega_k=0)$ and~marginalized neutrino mass density. They compared constraints from the~DES data alone (black contours) to~the~best available external data (green contours), but also showed the~impact of~including a~low-redshift SNe Ia data set (Low-$z$) to~anchor the~DES SNe Ia (blue contours). Each component of~the~DES analysis was fully blinded. Reprinted from \textcite{2019PhRvL.122q1301A}.}}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:des_res}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\\\textbf{\DIFdel{Euclid}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdel{Euclid is an~ESA (European Space Agency) high-precision space mission in~ESA's Cosmic Vision 2015 -- 2025 scientific program designed to~map the~geometry and~evolution of~the~universe and~to~study properties of~dark matter and~dark energy. Its primary goal is to~place high accuracy constraints on~dark energy, dark matter, gravity, and~cosmic initial conditions using two independent cosmological probes -- weak gravitational lensing and~baryonic acoustic oscillation -- out to~redshift $z\sim2$. Galaxy clusters and~the~Integrated Sachs--Wolfe effect will be used as secondary cosmological probes. Along with~these tasks will Euclid's visible and~near-infrared imaging and~spectroscopy of~the~entire extragalactic sky produce legacy science for~various fields of~astronomy, e.g. galaxy evolution, large-scale structures, or the~search for~high-redshift objects. The~Euclid mission has been adopted with~the~mission's time-frame for~liftoff starting in~mid-2022 from the~Guiana Space Centre, Europe's Spaceport in~French Guiana. The~overview of~the~Euclid system design and~scientific requirements can be found in~\mbox{%DIFAUXCMD
\cite{2011arXiv1110.3193L}}\hspace{0pt}%DIFAUXCMD
.
}%DIFDELCMD < \begin{figure}[htb]
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=0.55\textwidth]{cosmo_surveys/euclid.jpg}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Structural and~thermal model of~the~Euclid satellite.}%DIFDELCMD < \\%%%
\textit{\DIFdelFL{Credit:}} %DIFAUXCMD
%DIFDELCMD < \url{https://www.esa.int/}%%%
}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:euclid}
%DIFDELCMD < \end{figure}
%DIFDELCMD < \clearpage
%DIFDELCMD < %%%
\\\textbf{\DIFdel{Vera C. Rubin Observatory}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdel{The~Vera C. Rubin Observatory project, previously known as the~Large Synoptic Survey Telescope, will conduct the~10-year Legacy Survey of~Space and~Time (LSST, \mbox{%DIFAUXCMD
\cite{lsst}}\hspace{0pt}%DIFAUXCMD
). LSST is a~ground-based telescope being built in~northern Chile on~the~Cerro Pach\'{o}n mountain in~the~northern Chilean Andes. The~system will produce a~6-band (300 -- 1100 nm) wide-field deep astronomical survey over 20,000 deg\sq\ of~the~southern sky. Combining the~wide-field of~view with~short exposures, the~LSST will take more than 800 images each night and~cover the~whole observable sky twice each week. Each patch of~the~sky will be visited about 1000 times during ten years. The~LSST will provide an~unprecedented depth (single-visit 24.5 mag, co-added 27.5 mag) and~unique details of~the~Universe while producing 30 terabytes of~data nightly. This data will be used for~locating dark matter and~to~characterize the~properties of~the~dark energy. Other major tasks for~the~LSST will be detecting and~tracking potentially hazardous asteroids or studying the~structure of~the~Milky Way. The~project is in~the~construction phase and~will begin its full science operations in~2022. The~exterior of~the~summit facility building is nearly complete, see }%DIFDELCMD < \autoref{fig:lsst_site}%%%
\DIFdel{. Inside, major components of~the~telescope have begun to~arrive from their places of~manufacture around the~globe, including the~mirror washing and~coating equipment, and~the~Secondary Mirror (M2) and~its support system.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{On~January 6, 2020, it was announced that the~LSST will be named the~NSF (National Science Foundation) Vera C. Rubin Observatory (Rubin Observatory) after an~astronomer who provided important evidence of~the~existence of~dark matter. NSF also announced on~January 6, 2020, that the~telescope at~the~Rubin Observatory will be named the~Simonyi Survey Telescope in~recognition of~the~significant private donation made through the~Corporation early in~the~construction phase in~support of~the~design, development, and~fabrication of~the~telescope's primary mirror.
}%DIFDELCMD < \begin{figure}[htb]
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=0.7\textwidth]{cosmo_surveys/lsst_site.jpg}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Construction on~the~Vera C. Rubin Observatory. Most of~the~large pieces of~equipment have arrived on~the~summit, and~the~installation of~the~Telescope Mount Assembly (TMA) began in~early 2020. }\textit{\DIFdelFL{Credit:}} %DIFAUXCMD
%DIFDELCMD < \url{https://www.lsst.org/}%%%
}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:lsst_site}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The~LSST Data Management (DM) is responsible for~creating the~software, services, and~systems which will be used to~produce LSST's data products. DM is required to~generate and~process a~set of~data products and~to~make them available to~scientists and~the~public }%DIFDELCMD < \parencite{LSST_DM}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\textbf{\DIFdel{Telescope and~camera}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdel{The~LSST telescope consists of~three aspheric mirrors -- an~8.4-m primary M1, a~3.4-m convex secondary M2 (the~largest convex mirror ever made), and~a~5.0-m tertiary M3. The~primary mirror is highly annular having an~outer clear aperture of~8.36 m and~an~inner diameter of~5.12 m, giving an~effective collecting area of~a~6.67-m filled aperture. The~camera body and~its associated readout electronics are located in~the~1.8-m diameter hole in~the~secondary mirror.  The~hole in~the~tertiary mirror is used to~mount equipment for~the~maintenance of~the~LSST optical alignment. The~primary and~tertiary mirrors form a~continuous surface without any vertical discontinuities. The~M1-M3 monolith (see }%DIFDELCMD < \autoref{fig:lsst_m1m3}%%%
\DIFdel{) was completely formed and~polished in~February 2015 and~in~early 2019 it was shipped to~the~summit facility on~Cerro Pach\'{o}n.
}%DIFDELCMD < \begin{figure}[htb]
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=0.7\textwidth]{cosmo_surveys/lsst_m1m3.jpg}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{The~LSST Primary/Tertiary Mirror (M1M3) in~the~Richard F Caris Mirror Lab at~the~University of~Arizona for~optical testing. In~May 2019, the~mirror reached its new home in~the~Andes Mountains of~Northern Chile.}%DIFDELCMD < \\%%%
\textit{\DIFdelFL{Credit:}} %DIFAUXCMD
%DIFDELCMD < \url{https://www.lsst.org/}%%%
}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:lsst_m1m3}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The~LSST camera with~the~size of~1.6 meters by 3 meters and~the~weight of~2800 kilograms will be the~largest digital camera ever constructed. It will produce data of~extremely high quality with~minimal downtime and~maintenance. It is a~large-aperture, wide-field optical (0.3 -- 1 }%DIFDELCMD < \um%%%
\DIFdel{) imager designed to~provide a~3.5\textdegree\ field of~view with~sampling better than 0.2\arcsec. The~image surface is flat with~a~diameter of~approximately 64 cm. Used detectors are 16 Mpixel silicon detectors providing a~total of~approximately 3.2 Gpixels with~readout in~2 sec (15 sec integration). The~camera has 6 filters ($ugrizy$) and~is located in~the~middle of~the~telescope.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The~focal plane consists of~189 arrays ($\sim$16 cm\sq\ each, 3200 cm\sq\ focal plane) of~4k$\times$4k CCDs which should ensure a~wide-field of~view while filling the~focal plane without any large gaps (less than a~few hundred }%DIFDELCMD < \um%%%
\DIFdel{) -- the~fill factor is 93\%. High resistivity silicon substrate and~high applied voltages with~small pixel sizes will produce low point spread function (PSF $\ll0.7$\arcsec). Other focal plane requirements include high quantum efficiency (QE) from 320 to~1080 nm, fast f/1.23 focal ratio, high throughput fast readout (2 sec), or low read noise }%DIFDELCMD < \parencite{2017JInst..12C3017A}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\textbf{\DIFdel{Dark Energy Science Collaboration}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdel{The~LSST Project Team has been assembled to~design and~build the~telescope, camera, and~data management systems but it is not a~scientific collaboration in~the~usual sense. While scientists working on~the~LSST Project are interested in~the~scientific questions that LSST data can address, they are not officially involved in~the~scientific analyses of~those data and~have no privileged access to~LSST data or software. Therefore, a~number of~quasi-independent scientific collaborations, which provided advices on~technical issues and~helped articulate the~scientific case for~the~LSST, have created the~LSST Dark Energy Science Collaboration (DESC) in~2012 during a~meeting at~the~University of~Pennsylvania.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{DESC prepares a~variety of~cosmological analyses for~the~LSST survey. In~advance of~LSST's first observations, the~DESC helps prepare for~the~LSST science analysis, make synergistic connections with~ongoing cosmological surveys, and~provide the~dark energy community with~state of~the~art analysis tools. The~primary goal of~the~DESC is the~study of~dark energy and~related topics in~fundamental physics with~data from the~LSST. For~more information see the~DESC's white paper \mbox{%DIFAUXCMD
\cite{desc_white}}\hspace{0pt}%DIFAUXCMD
.
}%DIFDELCMD < 
...
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdel{The~Planck mission (\mbox{%DIFAUXCMD
\cite{planck}}\hspace{0pt}%DIFAUXCMD
...
%DIFDELCMD < \begin{figure}[htb]
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=0.4\textwidth]{cosmo_surveys/planck_s.jpg}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{The~Planck satellite. }\textit{\DIFdelFL{Credit:}} %DIFAUXCMD
%DIFDELCMD < \url{https://sci.esa.int/}%%%
}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:planck}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdel{The~Planck spacecraft was 4.2 meters high and~had a~maximum diameter of~4.2 meters, with~a~launch mass of~around 1.9 tonnes. The~spacecraft comprised a~service module, which housed systems for~power generation and~conditioning, attitude control, data handling and~communications, together with~the~warm parts of~the~scientific instruments, and~a~payload module. The~payload module consisted of~the~telescope, the~optical bench, with~the~parts of~the~instruments that needed to~be cooled - the~sensitive detector units - and~the~cooling systems.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Since the~end of~the~mission in~October 2013, the~Planck Collaboration has subsequently released all their data which now constitute the~Planck Legacy Archive (PLA) containing all public products originating from the~Planck mission. On~17 July 2018 ESA and~the~Planck Collaboration have released to~the~public a~new and~improved version of~the~data acquired by the~Planck satellite, which constitutes the~final official release from Planck.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The~data from Planck have allowed cosmologists to~set very tight constraints on~many parameters of~the~standard model, including the~Hubble constant, the~densities of~baryonic matter, dark matter and~dark energy, and~the~spectral index (for~Planck results, see \mbox{%DIFAUXCMD
\cite{planck_cosm}}\hspace{0pt}%DIFAUXCMD
).
}\\\textbf{\DIFdel{Nancy Grace Roman Space Telescope}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdel{The~Nancy Grace Roman Space Telescope (formerly known as WFIRST, the~Wide Field Infrared Survey Telescope)is a~NASA large space mission designed to~settle essential questions in~dark energy, exoplanets, and~infrared astrophysics. It is designed to~perform wide-field imaging and~slitless spectroscopic surveys of~the~near-infrared sky. The~current Astrophysics Focused Telescope Assets (AFTA) design of~the~mission makes use of~an~existing 2.4-m telescope to~enhance sensitivity and~imaging performance. It is the~top-ranked large space mission in~the~New Worlds, New Horizon (NWNH) Decadal Survey of~Astronomy and~Astrophysics. The~main instrument is a~wide-field multi-filter NIR imager and~spectrometer. With~the~2.4-m telescope, a~coronagraph instrument has been added to~the~payload for~direct imaging of~exoplanets and~debris disks. WFIRST was approved for~development and~launch in~2016 and~the~mission should start in~the~mid-2020s \mbox{%DIFAUXCMD
\cite{WFIRST_report}}\hspace{0pt}%DIFAUXCMD
.
}%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[htb]
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=0.7\textwidth]{cosmo_surveys/WFIRST.jpg}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Primary Mirror Assembly. }\textit{\DIFdelFL{Credit:}} %DIFAUXCMD
%DIFDELCMD < \url{https://www.nasa.gov/}%%%
}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:wfirst}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdel{The~mission will feature strategic key science programs plus a~large program of~guest observations. The~main focus is on~the~dark energy and~fundamental cosmology (determine the~expansion history of~the~Universe and~the~growth history of~its largest structures). The~next scientific goal is discovering of~exoplanets -- by microlensing photometric survey of~the~Galactic bulge and~by a~direct high-contrast imaging and~spectroscopic survey of~the~nearest stars. Data for~general astrophysics science will be gathered by surveys at~high Galactic latitudes and~Galactic bulge. A~relatively huge priority is assigned to~the~guest observer science program.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The~payload features a~2.4-m telescope, which feeds the~wide-field instrument (wide-field channel and~an~integral field unit spectrograph channel) and~the~coronagraph instrument. The~wide-field channel covers a~wavelength range of~0.76 -- 2.0 }%DIFDELCMD < \um %%%
\DIFdel{and~a~spectroscopy mode covering 1.35 -- 1.89 }%DIFDELCMD < \um%%%
\DIFdel{. The~wide-field focal plane uses 18 4k $\times$ 4k HgCdTe detector arrays. The~integral field unit channel uses an~image slicer and~spectrograph to~provide individual spectra of~each slice covering the~0.6 -- 2.0 }%DIFDELCMD < \um %%%
\DIFdel{spectral range. The~coronagraph instrument provides high-contrast imaging and~spectroscopy. Direct imaging is provided over a~band-pass of~430 -- 970 nm, while spectroscopy is provided by the~spectrograph over the~spectral range of~0.6 -- 0.97 }%DIFDELCMD < \um %%%
\DIFdel{with~a~spectral resolution of~$R\sim70$ \mbox{%DIFAUXCMD
\cite{WFIRST_report}}\hspace{0pt}%DIFAUXCMD
.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{As WFIRST will be a~NIR mission it will require visible band photometry for~photo-z determination. LSST will be the~premier ground-based facility to~provide those data. As for~Euclid, the~baryon acoustic oscillation spectroscopic survey will be helpful for~calibrating photo-z determinations for~LSST. The~comparison of~shear determinations between WFIRST and~LSST measurements will be useful for~understanding shape measurement systematics with~both facilities.}%DIFDELCMD < \clearpage{}
%DIFDELCMD < \clearpage{}%%%
\DIFdelend \chapter{Outlook}
\label{chpt:outlook}
