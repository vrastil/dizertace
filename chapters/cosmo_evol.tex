...
where the sound velocity $c_s$ is defined ad
\eq{
...
where $\bm k$ is a wavenumber with modulus $k$ and hat represents quantities in Fourier space. Since we are dealing with linear equations the different modes are not coupled and we can solve the equation for each $k$ independently.
...
\eq{
    \label{eq:lin_k_1}
    k^2\hat\Phi + 3\mathcal{H}(\hat\Phi'-\mathcal{H}\hat\Psi) &= 4\pi G\bar\rho a^2 \hat\delta\,,\\
    \label{eq:lin_k_2}
    k^2(\hat\Phi' - \mathcal{H}\hat\Psi) &= -4\pi G\bar\rho a^2(1+\omega)\hat\theta\,,\\
    \label{eq:lin_k_3}
    \hat\Psi &=-\hat\Phi\,,\\
    \label{eq:lin_k_4}
    \hat\Phi''+2\mathcal{H}\hat\Phi'-\mathcal{H}\hat\Psi-(\mathcal{H}^2+2\mathcal{H}')\hat\Psi &=-4\pi G\bar\rho a^2c^2_s\hat\delta\,,\\
    \label{eq:lin_k_5}
    \hat\delta'+3\mathcal{H}(c_s^2-\omega)\hat\delta &= -(1+\omega)(\hat\theta+3\hat\Psi')\,,\\
    \label{eq:lin_k_6}
    \hat\theta'+\left[\mathcal{H}(1-3\omega)+\frac{\omega'}{1+\omega}\right]\hat\theta &= k^2\left(\frac{c_s^2}{1+\omega}\hat\delta + \hat\Psi \right)\,,
}
where now $\hat\theta=i\bm{k\cdot \hat v}$. Note that these six equations are not independent and $\omega$ and $c_s^2$ are arbitrary functions of time.
