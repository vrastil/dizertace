...
where $\tau$ is the proper time and $v^i=\dd x^i/\dd\eta$ is the matter peculiar velocity with respect to the general expansion. The energy-momentum tensor for a perfect fluid is
\eq{
    T_\uv=(\rho+p)u_\mu u_\nu + Pg_\uv\,.
}
We also assume that the perturbed fluid remains perfect fluid. We will use following notation
\eq{
...
\eq{
    ...
    \nabla^2(\Phi' - \mathcal{H}\Psi) &= 4\pi G a^2(1+\omega)\rho\theta\,,\\
    \Psi &=-\Phi\,,\\
    \Phi''+2\mathcal{H}\Phi'-\mathcal{H}\Psi-(\mathcal{H}^2+2\mathcal{H}')\Psi &=-4\pi Ga^2c^2_s\delta\rho\,,
}
