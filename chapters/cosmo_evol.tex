...
where $g_{\mu\nu}$ is a metric tensor, $a(t)$ is a scale factor with cosmic time $t$ and $K=+1,-1,0$ is a curvature that corresponds to closed, open, and flat geometries. In addition to the cosmic time $t$, we also introduce the conformal time $\eta$ defined by
\eq{
...
\eq{
    z\equiv\frac{\lambda_0}{\lambda} - 1\,.
}
