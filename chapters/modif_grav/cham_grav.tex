...
where $T^{(i)}_\uv$ is the~stress-energy tensor for~the~$i$-th matter component. The~transformation from the~Jordan frame to~the~Einstein frame introduces non-standard coupling of~the~chameleon field to~standard matter resulting in~the~fifth force \eqref{cham_force}. Using our variables, this equation reads
\eq{
	\label{eq:cham_force}
