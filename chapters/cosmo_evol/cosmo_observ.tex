...
\label{sec:bao}
Baryonic acoustic oscillations (BAO) provide a way of~measuring cosmic distance and~hence they can be~used to~constraining dark energy and~cosmic geometry. Sound waves propagating before recombination imprint a~characteristic scale on~matter clustering. The~acoustic length scale can be computed as
\eq{
...
BAO can be clearly seen in~the~correlation function \parencite{1993ApJ...412...64L}. The~two-point correlation function $\xi(r)$ is the~excess probability $\dd P$ of~finding two pairs of~galaxies in~two volumes $\dd V_1$ and~$\dd V_2$ at~a~given comoving distance $r$
\eq{
    \label{eq:corr}
...
%%%%%%%%%%%%%%%%%%%%%%%
\subsection[Large-scale structure]{Large-scale structure}
\label{ssec:lss}
Studying the~large-scale structures (LSS) of~the~Universe is of~great importance for~the~cosmology, since the~LSS encode critical information about the~contents of~the~Universe, the~origin of~the~fluctuations, and~the~cosmic expansion background in~which the~structures evolved.
...
% Galaxy clusters
%%%%%%%%%%%%%%%%%%%%%%%
% \subsection[Galaxy clusters]{Galaxy clusters}
% \label{ssec:gc}
...
% }
% As their range of~masses and~redshifts correspond to~our study case we will use this fitting formula in~our analysis.
%%%%%%%%%%%%%%%%%%%%%%%
% Strong lensing
%%%%%%%%%%%%%%%%%%%%%%%
